...
其中，将动量衰减参数 β 设置为 0.9 是超参数的一个常见且效果不错的选择。
...
而使用动量梯度下降时，通过累加减少了抵达最小值路径上的波动，加速了收敛，因此在横轴方向下降得更快，从而得到图中红色的曲线。
...
**Adam 优化算法（Adaptive Moment Estimation，自适应矩估计）**基本上就是将 Momentum 和 RMSprop 算法结合在一起。具体过程如下：
...
（可以看到 Andrew 在这里 ϵ 没有写到平方根里去，和他在 RMSprop 中写的不太一样。基于 ϵ 的作用，我感觉问题不大）
